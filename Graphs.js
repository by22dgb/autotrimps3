...
function formatDuration(timeSince) {
  ...
  for (const [unit, value] of Object.entries(timeObj)) {
    ...
    if (value) {
      ...
      timeString += value.toString() + CN_unit + "";
    }
  }
  if (unitsUsed <= 1) {
    timeString = [timeObj.s.toString().padStart(1, "0"), milliseconds.toString(), "秒"].join(".");
  }
  return timeString
}
...
const formatters = {
  datetime: function () {
    let ser = this.series;
    return '<span style="color:' + ser.color + '" >●</span>' + ser.name + "<i></i>：<b>" + formatDuration(this.y / 1000) + "</b><br>";
  },
  defaultPoint: function () {
    var ser = this.series; // 'this' being the highcharts object that uses formatter()
    return '<span style="color:' + ser.color + '" >●</span>' + ser.name + "<i></i>：<b>" + prettify(this.y) + "</b><br>";
  },
...
function createUI() {
  ...
  chartscript.src = "https://by22dgb.github.io/autotrimps3/highcharts.js";
...
function Graph(dataVar, universe, selectorText, additionalParams = {}) {
  ...
  this.lineGraph = function () {
    ...
    for (const portal of Object.values(portalSaveData).reverse()) {
      ...
      this.graphData.push({
        name: `传送门${portal.totalPortals}：<i></i>${portal.challenge}`,
        data: cleanData,
      })
      portalCount++;
...
const graphList = [
  ...
  new Graph(false, false, "Portal Stats", {
    ...
    columns: [
      ...
      { dataVar: "totalNullifium", title: "虚空物质", color: "#8a008a" },
...
const toggledGraphs = {
  ...
  perZone: {
    graphMods: (graph, highChartsObj) => {
      highChartsObj.title.text = "每区域的<i></i>" + highChartsObj.title.text
  ...
  perHr: {
    graphMods: (graph, highChartsObj) => {
      highChartsObj.title.text = "每小时的<i></i>" + highChartsObj.title.text
    },
  ...
  lifetime: {
    graphMods: (graph, highChartsObj) => {
      highChartsObj.title.text += "<i></i>总数的百分比";
      highChartsObj.yAxis.title.text += "<i></i>总数的百分比"
    },
    customFunction: (portal, item, index, x) => {
      var initial;
  ...
  s3normalized: {
    graphMods: (graph, highChartsObj) => {
      var maxS3 = Math.max(...Object.values(portalSaveData).map((portal) => portal.s3).filter((s3) => s3));
      highChartsObj.title.text += `<i></i>，归一化至区域${maxS3}的污污等级3技能`
    },
