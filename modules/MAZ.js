...
function mapSettingsHelpWindow(titleText, activeSettings) {
	...
	*/
	if (!s.golden && !s.profile) {
		...
		if (s.mapBonus) {
			...
			const settingText = showSpire ? `either the <b>Map Bonus Ratio</b>或尖塔<b>Max Map Bonus</b>` : `the <b>Map Bonus Ratio</b>`;
		...
		if (s.voidMap) {
			...
			mazHelp += `<li class="indent">Will only farm <b>Void Hits Survived</b> to this value when set <b>超过0的数值</b>.</li>`;
			...
			mazHelp += `<li class="indent">Will only farm <b>Void HD Ratio</b> to this value when set <b>超过0的数值</b>.</li>`;
		...
		if (s.boneShrine) {
			...
			mazHelp += `<li class="indent">If set to <b>0 or below</b> OR <b>超过10的数值</b> it will stop bone charges from being auto spent.</li>`;
		...
		if (s.hdFarm) {
			const showSpire = (!radonSetting && hze >= 170) || (radonSetting && hzeU2 >= 270);
			const settingText = showSpire ? `either the <b>Hits Survived</b>或尖塔<b>Hits Survived</b>` : `the <b>Hits Survived</b>`;
	...
	*/
	...
	if (s.endZone && !s.voidMap) {
		...
		if (s.repeatEvery) mazHelp += `<li class="indent">This input is only used when your <b>Repeat Every</b> input is <b>超过0的数值</b>时生效。</li>`;
	}
	...
	if (s.mapLevel) {
		...
		if (radonSetting && !(s.mapBonus || s.insanity || s.alchemy || s.hypothermia)) mazHelp += `<li class="indent">在<b>Wither</b>挑战中将无视<b>-1</b>以上的输入值。</li>`;
	}
	...
	*/
	...
	if (s.spireAssault) {
		...
		mazHelp += `<li class="indent">Item presets can be setup in the <b>Item Presets</b> setting in the <b>Spire Assault</b>选项卡)。</li>`;
...
function autoStructureTable(settingGroup, hze) {
	...
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Buildings</span><br>`;
...
function autoJobsDisplay(elem) {
	...
	autoRatios += `<br><b>3/1/4</b> - When above <i></i>${prettify(3e6)}<i></i>之后。`;
	autoRatios += `<br><b>3/3/5</b> - When above <i></i>${prettify(3e5)}<i></i>之后。`;
	autoRatios += `<br><b>1/1/2</b> - Used running a map until you reach <i></i>${prettify(3e6)}<i></i>，运行地图时。`;
...
function autoJobsTable(settingGroup, ratioJobs, percentJobs) {
	let tooltipText = `<div id='autoStructure'>`;
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Jobs</span><br>`;
	...
	for (let item in ratioJobs) {
		...
		<div id ='${item}RatioDiv' style='display: flex; align-items: center; margin-bottom: 0.1em;'>
			<span id='${item}TextBox' class='textbox' style='text-align: left; height: 1.5vw; max-width: 9vw; min-width: 9vw; font-size: 0.7vw; margin-right: 0.4em;' onclick='document.getElementById("${item}Ratio").focus()'>Ratio:<i></i>&nbsp;&nbsp;&nbsp;&nbsp;
...
function uniqueMapsTable(settingGroup, mapUnlocks, smithySettings) {
	...
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Unique Maps</span><br>`;
	...
	if (Object.keys(smithySettings).length > 0) {
		...
		tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Smithy Melting Point</span><br>`;
...
function dailyPortalModsDisplay(elem) {
	...
	const tooltips = {
		Reflect: `敌人有x%概率反馈一次攻击，将本次攻击造成伤害的y%反馈给我方脆皮。(反馈伤害不会超过该敌人的生命值上限)。`,
		Empower: '[赋能]当脆皮在世界中阵亡时，世界上所有敌人获得x层“赋能”。每层赋能使世界上敌人的攻击力和生命值增加0.2%，层数最高为9999层，在本次挑战过程中层数不会重置。',
		Mutimp: '世界上前x行40%的敌人将变异为脆皮(大型)变异者。',
		Bloodthirst: '[嗜血]当脆皮阵亡时敌人获得一层“嗜血”。每达到x层嗜血时，使敌人恢复所有生命值，并增加50%攻击力(相互叠加)。层数最高为y层，在击杀一名敌人后重置。',
		Famine: '使金属、食物、木头和宝石的获取量减少x%。',
		Large: '使所有住房少容纳x%的脆皮。',
		Weakness: '[弱化]敌人每次攻击后叠加一层“弱化”，每层使我方脆皮的攻击力减少x%，层数最高为9层，在我方脆皮阵亡后层数重置。',
		Empowered_Void: '使虚空地图中敌人的攻击力和生命值增加x%。',
		Heirlost: '使传家宝与战斗和资源获取速度相关的加成减少x%。'
	};
	...
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Daily Modifiers</span><br>`;
...
function c2RunnerDisplay(elem) {
	...
	tooltipText += `<div id='baseChallenges'>`;
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Challenges</span><br>`;
	const elemWidth = 'calc((100% - 1.4em - 2px) / 6.1)';
	...
	for (let item in settingGroup) {
		if (fusedC2s.includes(item) && !addedFusedHeader) {
			...
			tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Fused Challenges</span></span><br/>`;
		...
		rowData += `
			<div id='${item}Btn' class='btnItem btnItem${equipClass}' style='height: 1.5vw; max-width: ${elemWidth}; min-width: ${elemWidth}; margin-right: 0.1em; margin-left: 0.1em; margin-top: 0.1em; margin-bottom: 0.2em;' onclick='hideAutomationToggleElem(this)' data-hidden-text="${item}">
				...
				<span style="float: right;">区域${challengeLevel}</span>
			</div>&nbsp;`;
...
function hideAutomationDisplay(elem) {
	...
	tooltipText += `<div id='baseGameItems'>`;
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>Base Game Features</span></span><br/>`;
	...
	tooltipText += `<span class='messageConfigContainer' style='font-size: 1.3vw;'>&nbsp;<i></i>AutoTrimps Features</span></span><br/>`;
...
function hideAutomationConfigHover(what, event, hide = false) {
	...
	const messageConfigMap = {
		...
		ATheHr: { title: `${heliumOrRadon()}每小时状态`, text: `隐藏${heliumOrRadon()}每小时状态的消息。` }
	};
