...
function autoRoboTrimp(){if(!(0<game.global.roboTrimpCooldown)&&game.global.roboTrimpLevel){var a=parseInt(getPageSetting("AutoRoboTrimp"));0==a||game.global.world>=a&&(game.global.world-a)%5==0&&!checkIfLiquidZone()&&!game.global.useShriek&&(magnetoShriek(),MODULES.other.enableRoboTrimpSpam&&debug("在区域"+game.global.world+"激活了小脆皮机器人的电磁尖啸","graphs","*podcast"))}}
...
function BWraiding() {
  ...
  if (isBWRaidZ && !bwraided && !failbwraid && getPageSetting(bwraidSetting)) {
    ...
    if (findLastBionic().level > targetBW && !bwraided && !failbwraid) {
      ...
      failbwraid = false;
      bwraidon = false;
      if (game.options.menu.climbBw.enabled)
        repeatClicked();
