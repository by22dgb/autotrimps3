...
function initializeAllSettings() {
    ...
    createSetting('CustomAutoPortal', '自定义传送', '在通过相应区域*后*自动传送。(例如：设为200后，将在到达区域201时传送)', 'value', '999', null, "Core");
    ...
    createSetting('RCustomAutoPortal', '自定义传送', '在通过相应区域*后*自动传送。(例如：设为200后，将在到达区域201时传送)', 'value', '999', null, "Core");
    ...
    //Line 2
    createSetting('DisableFarm', '刷资源命攻比', 'If H:D goes above this value, it will farm for Damage & Health. The lower this setting, the more it will want to farm. Default is <b>16</b>。<b>设为-1则关闭刷资源模式！</b>', 'value', -1, null, 'Maps');
    ...
    createSetting('RDisableFarm', '刷资源命攻比', 'If H:D goes above this value, it will farm for Damage & Health. The lower this setting, the more it will want to farm. Default is <b>16</b>. <b>设为-1则关闭刷资源模式！</b>', 'value', -1, null, 'Maps');
    ...
    createSetting('Rmeltpoint', '熔点', '设为-1则关闭该选项。设置运行熔点地图的时机。可以输入两个数值，格式类似50,91这样。第一个数字用于设置运行熔点地图的区域，第二个数字用于设置运行熔点地图的格子。前面的例子中，脚本将在区域50的格子91运行熔点地图。必须设置两个数值才能生效。用于熔化挑战和捕手道达人挑战。', 'multiValue', [-1], null, 'Maps');
    createSetting('Rfrozencastle', '非失温冻结城堡', '设为-1则关闭该选项。设置运行冻结城堡地图的时机。可以输入两个数值，格式类似200,91这样。第一个数字用于设置运行冻结城堡地图的区域，第二个数字用于设置运行冻结城堡地图的格子。前面的例子中，脚本将在区域200的格子91运行冻结城堡地图。必须设置两个数值才能生效。用于失温挑战以外的情况，请小心设置。', 'multiValue', [-1], null, 'Maps');
