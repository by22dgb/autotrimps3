...
function initializeAllTabs() {
    ...
    createTabs("C2", "挑战²和挑战³ - 挑战²和挑战³相关的设置");
...
function initializeAllSettings() {
    ...
    createSetting('CustomAutoPortal', '自定义传送', '在通过相应区域*后*自动传送。(例如：将数值设为200后，将在到达区域201时传送)', 'value', '999', null, "Core");
    ...
    createSetting('RCustomAutoPortal', '自定义传送', '在通过相应区域*后*自动传送。(例如：将数值设为200后，将在到达区域201时传送)', 'value', '999', null, "Core");
    ...
    createSetting('RadonHrBuffer', '氡/小时缓冲值百分比', 'IMPORTANT SETTING. When using the Rn/Hr Autoportal, it will portal if your Rn/Hr drops by this amount of % lower than your best for current run, default is 0% (ie: set to 5 to portal at 95% of your best). Now with stuck protection - Allows portaling midzone if we exceed set buffer amount by 5x. (ie a normal 2% buffer setting would now portal mid-zone you fall below 10% buffer).', 'value', '0', null, 'Core');
    ...
    //Line 1
    createSetting('buyheliumy', '氦奇购买百分比', 'Buys the Heliumy bonus for <b>100 bones</b> when Daily bonus is above the value set in this setting. Recommend anything above 475. Will not buy if you cant afford to, or value is -1. ', 'value', -1, null, 'Daily');
    ...
    createSetting('dIgnoreSpiresUntil', '日常尖塔无效区域', 'Spire specific settings like end-at-cell are ignored until at least this zone is reached in Dailies (0 to disable). ', 'value', '200', null, 'Daily');
    createSetting('dExitSpireCell', '日常塔退出格子', 'What cell to exit spire in dailys. ', 'value', -1, null, 'Daily');
    createSetting('dPreSpireNurseries', '日常进塔前托儿所', 'Set the maximum number of Nurseries to build for Spires in Dailies. Overrides No Nurseries Until z and Max Nurseries so you can keep them seperate! Disable with -1.', 'value', -1, null, 'Daily');
    ...
    createSetting('dwsmax', '日常风层数叠加上限', 'For maximising Windstacking an entire Daily. Withholds damage to try and get your max windstacks every wind zone. Not recommended for terrible Dailies. ', 'value', '-1', null, 'Daily');
    createSetting('dwsmaxhd', '日常风层数叠加上限命攻比', 'Fiddle with this to maximise your DWSM settings. Default is 0.00025. ', 'value', '-1', null, 'Daily');
    
    //Raiding
    document.getElementById('dwsmaxhd').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('dPraidingzone', '日常重铸劫掠区域', 'Raids Maps for prestiges at zone specified in Dailies. Example: 495, will raid Maps at 501. Once all gear is obtained from the map, it will revert back to regular farming. Extremely helpful for spire. Best used in poison zones. <b>You can use multiple values like this 495,506,525! </b>', 'multiValue', [-1], null, 'Daily');
    createSetting('dPraidingcell', '日常重铸劫掠格子', '设置开始重铸劫掠的格子。如果还设置了“日常仿生劫掠格子”，建议设置的数值比它的数值低。将数值设为-1则在格子1开始劫掠。', 'value', -1, null, 'Daily');
    createSetting('dPraidingHD', '日常重铸劫掠命攻比', 'Checks if you can raid the map. If your HD value (calculated using the maps you will raid) is below this value it will not buy the map and you will stop raiding. The higher this value the higher zones it will raid. Can raid up to +10 depending on the zone. -1 or 0 to remove this check.', 'value', -1, null, 'Daily');
    createSetting('dPraidingP', '日常重铸劫掠毒上限', 'Maximum level of map to P Raid at in Poison. If this value is 10 it will be able to go to +10 maps in Poison. You should use this instead of the HD function if you feel the calculations are off, but you can use both if needed. -1 or 0 to have no max. ', 'value', -1, null, 'Daily');
    createSetting('dPraidingI', '日常重铸劫掠冰上限', 'Maximum level of map to P Raid at in Ice. If this value is 10 it will be able to go to +10 maps in Ice. You should use this instead of the HD function if you feel the calculations are off, but you can use both if needed. -1 or 0 to have no max. ', 'value', -1, null, 'Daily');
    ...
    createSetting('Rdarmormagic', ['日常护甲：关闭', '日常护甲：超过80%', '日常护甲：命攻比', '日常护甲：常时'], 'Will buy Armor to try and prevent death on Bleed/Plague/Bogged Dailies under the 3 conditions. <br><b>超过80%：</b> Will activate at and above 80% of your HZE. <br><b>H:D:</b> Will activate at and above the H:D you have defined in maps. <br><b>Always</b> Will activate always. <br>All options will activate at or <b>below 25% of your health.</b> ', 'multitoggle', 0, null, "Daily");
    ...
    createSetting('RdPraidingzone', '日常重铸劫掠区域', 'Raids Maps for prestiges at zone specified in Dailies. Example: 495, will raid Maps at 501. Once all gear is obtained from the map, it will revert back to regular farming. Extremely helpful for spire. Best used in poison zones. <b>You can use multiple values like this 495,506,525! </b>', 'multiValue', [-1], null, 'Daily');
    ...
    createSetting('RdMaxPraidZone', '日常重铸劫掠最大区域', 'List of maximum zones to Praid on Dailies corresponding to the list specified in Daily Praiding Z.  e.g. if Daily P raiding Z setting is 491,495 and this setting is 495,505, AT will P raid up to 495 from 491, and 505 from 495.  Set to -1 to always buy highest available prestige map.  If no corrsponding value, or value is invalid, defaults to max available (up to +10)', 'multiValue', [-1], null, 'Daily');
    createSetting('RdPraidFarmFragsZ', '日常刷碎片区域', 'P Raiding harder: List of zones where we should farm fragments until we can afford the highest or target prestige map for P raiding. Set to -1 to never farm fragments. ', 'multiValue', [-1], null, 'Daily');
    createSetting('RdPraidBeforeFarmZ', '日常刷碎片前劫掠区域', 'P Raiding harder: List of zones where we should P Raid as far as we can afford before trying to farm fragments to Praid the highest or target prestige map.  Only occasionally useful, e.g. if it picks up a Speedexplorer or farming fragments is slow due to low damage. Set to -1 to never raid prestiges before farming fragents.', 'multiValue', [-1], null, 'Daily');
    createSetting('RDailybwraid', 'Daily BW Raid', 'Toggle for Daily BW Raid settings. ', 'boolean', false, null, 'Daily');
    createSetting('RdBWraidingz', '日常仿生劫掠区域', 'Raids BWs at zone specified in dailys. Example: 495, will raid all BWs for all gear starting from 495. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Accepts comma separated lists, and raids up to the value in the corrsponding position in the Max BW to raid setting. So if this is set to 480,495 and Daily Max BW to Raid is set to 500,515 AT will BW raid up to 500 from 480, and 515 from 495. Make sure these lists are the same length or BW raiding may fail.', 'multiValue', [-1], null, 'Daily');
    createSetting('RdBWraidingmax', '日常仿生劫掠最大区域', 'Raids BWs until zone specified in dailys. Example: 515, will raid all BWs for all gear until 515. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Now accepts comma separated lists - see description of Daily Z to BW raid setting for details.', 'multiValue', [-1], null, 'Daily');

    
    //Heirloom
    createSetting('dhighdmg', '日常传家宝：高伤', '<b>HIGH DAMAGE HEIRLOOM</b><br><br>Enter the name of your high damage heirloom. This is your heirloom that you will use normally in dailies. ', 'textValue', 'undefined', null, 'Daily');
    ...
    createSetting('u1daily', '在宇宙1进行日常挑战', 'If this is on, you will do your daily in U1. ', 'boolean', false, null, 'Daily');
    ...
    createSetting('RdCustomAutoPortal', '日常自定义传送', '日常挑战中，在通过相应区域*后*自动传送。(例如：将数值设为200后，将在到达区域201时传送)', 'value', '999', null, "Daily");
    createSetting('RdHeHrDontPortalBefore', '日常自动传送区域', 'Do NOT allow Radon per Hour Daily AutoPortal setting to portal BEFORE this level is reached in dailies. It is an additional check that prevents drops in radon/hr from triggering autoportal in dailies. Set to 0 or -1 to completely disable this check. (only shows up with Radon per Hour set in dailies)', 'value', '999', null, "Daily");
    createSetting('RdHeliumHrBuffer', '日常氡/小时缓冲值百分比', 'IMPORTANT SETTING. When using the Daily Rn/Hr Autoportal, it will portal if your Rn/Hr drops by this amount of % lower than your best for current run in dailies, default is 0% (ie: set to 5 to portal at 95% of your best in dailies). Now with stuck protection - Allows portaling midzone if we exceed set buffer amount by 5x. (ie a normal 2% buffer setting would now portal mid-zone you fall below 10% buffer).', 'value', '0', null, 'Daily');
    createSetting('RDailyVoidMod', '日常虚空区域', '日常挑战中，运行所有虚空地图的区域。将数值设为-1则关闭该选项。', 'value', -1, null, 'Daily');
    createSetting('RdRunNewVoidsUntilNew', '日常新虚空变化值', '<b>0 to disable. Positive numbers are added to your Void Map zone. -1 for no cap.</b> This allows you to run new Void Maps in Dailies obtained after your Void Map zone by adding this number to your Void Map zone. <br> <b>Example</b> Void map zone=187 and This setting=10. New Voids run until 197).<br>This means that any new void maps gained until Z197. CAUTION: May severely slow you down by trying to do too-high level void maps. Default 0 (OFF).', 'value', '0', null, 'Daily');
    ...
    //Line 1
    createSetting('FinishC2', '自动完成挑战²', '<b>DONT USE THIS WITH C2 RUNNER</b><br>Finish / Abandon Challenge2 (any) when this zone is reached, if you are running one. For manual use. Recommended: Zones ending with 0 for most Challenge2. Disable with -1. Does not affect Non-Challenge2 runs.', 'value', -1, null, 'C2');
    createSetting('buynojobsc', '挑战²中不雇佣农/伐/矿', 'Buys No Farmers, Lumberjacks or Miners in the C2 challenges Watch and Trapper. ', 'boolean', 'false', null, "C2");
    ...
    createSetting('carmormagic', ['挑战²护甲：关闭', '挑战²护甲：超过80%', '挑战²护甲：命攻比', '挑战²护甲：常时'], 'Will buy Armor to try and prevent death on Nom/Tox Challenges under the 3 conditions. <br><b>超过80%：</b> Will activate at and above 80% of your HZE and when your health is sufficiently low. <br><b>H:D:</b> Will activate at and above the H:D you have defined in maps. <br><b>Always</b> Will activate always. <br>All options will activate at or <b>below 25% of your health.</b> ', 'multitoggle', 0, null, "C2");
    createSetting('mapc2hd', '地图学命攻比', 'Set your H:D ratio for Mapology. Will not go into maps unless your H:D ratio is above this. -1 to use normal behaviour. ', 'value', '-1', null, 'C2');
    ...
    createSetting('c2runnerportal', '挑战²自动传送', '在挑战²中通过相应区域*后*自动传送。(例如：将数值设为200后，将在到达区域201时传送)', 'value', '999', null, "C2");
    createSetting('c2runnerpercent', '挑战²自动运行百分比', 'What percent Threshhold you want C2s to be over. E.g 85, will only run C2s with HZE% below this number. Default is 85%. Must have a value set for C2 Runner to... well, run. ', 'value', '85', null, "C2");
    ...
    createSetting('FarmerRatio', '农民比例', '', 'value', '1', null, "Jobs");
    createSetting('LumberjackRatio', '伐木工比例', '', 'value', '1', null, "Jobs");
    createSetting('MinerRatio', '矿工比例', '', 'value', '1', null, "Jobs");
    createSetting('MaxScientists', '科学家上限', 'Advanced. Cap your scientists (This is an absolute number not a ratio). recommend: -1 (infinite still controls itself)', 'value', '-1', null, "Jobs");
    createSetting('MaxExplorers', '探险家上限', 'Advanced. Cap your explorers (This is an absolute number not a ratio). recommend: -1', 'value', '-1', null, "Jobs");

    //Line 2
    createSetting('MaxTrainers', '训练师上限', 'Advanced. Cap your trainers (This is an absolute number not a ratio). recommend: -1', 'value', '-1', null, "Jobs");
    ...
    createSetting('RFarmerRatio', '农民比例', '', 'value', '1', null, "Jobs");
    createSetting('RLumberjackRatio', '伐木工比例', '', 'value', '1', null, "Jobs");
    createSetting('RMinerRatio', '矿工比例', '', 'value', '1', null, "Jobs");
    createSetting('RMaxExplorers', '探险家上限', 'Advanced. Cap your explorers (This is an absolute number not a ratio). recommend: -1', 'value', '-1', null, "Jobs");
    ...
    createSetting('Rshipfarmzone', '刷崇：区域', 'Farms for specified worshippers in SF: Amount at zone according to this settings value. Can use 59,61,62. ', 'multiValue', [-1], null, 'Jobs');
    createSetting('Rshipfarmcell', '刷崇：格子', 'Ship Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Jobs');
    createSetting('Rshipfarmamount', '刷崇：数量', 'How many Worshippers to farm up to at zone specified in SF. Can use 50,45,40. These values should match up to your SF zones. If using SF and SF: Amount examples (59 and 50 respectively) it will farm at z59 up to 50 Worshippers. Just use one 50 if you want it to farm to 50 everytime. ', 'multiValue', [-1], null, 'Jobs');
    createSetting('Rshipfarmlevel', '刷崇：地图等级', 'What map level to use for SF. Can use -1,1,2. -1 to use a level down from world (Map Reducer mastery gives loot equal to world one level down), 0 to use world, 1 etc to use +maps. Using 0 by itself will use global level for all maps. ', 'multiValue', [0], null, 'Jobs');
    ...
    //Line 2
    createSetting('ForcePresZ', '强制重铸区域', 'On and after this zone is reached, always try to prestige for everything immediately, ignoring Dynamic Prestige settings and overriding that of Linear Prestige. Prestige Skip mode will exit this. Disable with -1.', 'value', -1, null, 'Gear');
    createSetting('PrestigeSkip1_2', ['关闭重铸跳过', '重铸跳过1和2', '重铸跳过1', '重铸跳过2'], '<b>重铸跳过1：</b> If there are more than 2 Unbought Prestiges (besides Shield), ie: sitting in your upgrades window but you cant afford them, AutoMaps will not enter Prestige Mode, and/or will exit from it. The amount of unboughts can be configured with this variable MODULES[\\"maps\\"].SkipNumUnboughtPrestiges = 2; <br><b>重铸跳过2：</b> If there are 2 or fewer <b>Unobtained Weapon Prestiges in maps</b>, ie: there are less than 2 types to run for, AutoMaps will not enter Prestige Mode, and/or will exit from it. For users who tends to not need the last few prestiges due to resource gain not keeping up. The amount of unboughts can be configured with MODULES.maps.UnearnedPrestigesRequired. If PrestigeSkipMode is enabled, both conditions need to be reached before exiting.', 'multitoggle', 0, null, "Gear");
    ...
    createSetting('gearamounttobuy', '每次购买装备等级', 'Set the amount of Gear Levels to buy for AT. I.e if set to 1 will buy 1 level at a time. Recommended value 1. <b>MUST ALWAYS HAVE A VALUE GREATER THAN 0! </b>', 'value', 1, null, "Gear");
    createSetting('always2', '常时等级2', 'Always buys level 2 of weapons and armor regardless of efficiency', 'boolean', false, null, "Gear");
    ...
    createSetting('Rdmgcuntoff', '装备：界限', 'Decides when to buy gear. 1 is default. This means it will take 1 hit to kill an enemy. If zone is below the zone you have defined in AE: Zone then it will only buy equips when needed. ', 'value', '1', null, 'Gear');
    createSetting('Requipamount', '装备：数量', 'How much equipment to level per time. ', 'value', 1, null, "Gear");
    createSetting('Requipcapattack', '装备：武器上限', 'What level to stop buying Weapons at. ', 'value', 50, null, "Gear");
    createSetting('Requipcaphealth', '装备：护甲上限', 'What level to stop buying Armour at. ', 'value', 50, null, "Gear");
    createSetting('Requipzone', '装备：区域', 'What zone to stop caring about H:D and buy as much prestiges and equipment as possible. ', 'value', -1, null, "Gear");
    createSetting('Requippercent', '装备：百分比', 'What percent of resources to spend on equipment before the zone you have set in AE: Zone', 'value', 1, null, "Gear");
    createSetting('Requip2', '装备：常时等级2', 'Always buys level 2 of weapons and armor regardless of efficiency', 'boolean', true, null, "Gear");
    ...
    createSetting('Requipfarmzone', '装刷：区域', 'What zone to start AEF: H:D and AEF: Multiplier. ', 'value', '-1', null, 'Gear');
    createSetting('RequipfarmHD', '装刷：命攻比', 'What H:D to use for AEF target. ', 'value', '-1', null, 'Gear');
    createSetting('Requipfarmmult', '装刷：倍率', 'Starting from the zone above AEF: Zone, this setting will multiply the H:D you have set in AEF: H:D. So if AEF: Zone was 100, AEF: H:D was 10, AEF: Multiplier was 1.2, at z101 your H:D target will be 12, then at z102 it will be 14.4 and so on. This way you can account for the zones getting stronger and you will not waste time farming for a really low H:D. ', 'value', '-1', null, 'Gear');
    createSetting('Requipfarmhits', '装刷：次数', 'How many hits do you want to kill an enemy in a AEF map. ', 'value', '-1', null, 'Gear');
    ...
    createSetting('MaxMapBonusAfterZone', '最大地图奖励起点', 'Always gets Max Map Bonus from this zone on. (inclusive and after).<br><b>NOTE:</b> Set -1 to disable entirely (default). Set 0 to use it always.<br><b>Advanced:</b>User can set a lower number than the default 10 maps with the AT hidden console command: MODULES[\\"maps\\"].maxMapBonusAfterZ = 9;', 'value', '-1', null, 'Maps');
    createSetting('MaxMapBonuslimit', '最大地图奖励界限', 'Limit the amount of Map Bonuses you get. Default is 10. ', 'value', '10', null, 'Maps');
    createSetting('MaxMapBonushealth', '最大地图奖励生命值', 'Limit the amount of map bonuses you get when AutoMaps requires more health. Default is 10. ', 'value', '10', null, 'Maps');
    createSetting('mapcuntoff', '地图界限', 'Decides when to get max map bonus. 4 is default. This means it will take 1 hit to kill an enemy if in D stance. ', 'value', '4', null, 'Maps');

    //Line 2
    createSetting('DisableFarm', '刷资源命攻比', 'If H:D goes above this value, it will farm for Damage & Health. The lower this setting, the more it will want to farm. Default is <b>16</b>。<b>将数值设为-1则关闭刷资源模式！</b>', 'value', -1, null, 'Maps');
    ...
    createSetting('VoidMaps', '虚空地图', '<b>0 to disable</b> The zone at which you want all your void maps to be cleared inclusive of the zone you type. Runs them at Cell 70. Use odd zones on Lead.<br>', 'value', '0', null, "Maps");
    createSetting('voidscell', '虚空格子', 'Run Voids at this Cell. -1 to run them at the default value, which is 70. ', 'value', '-1', null, 'Maps');
    createSetting('RunNewVoidsUntilNew', '新虚空变化值', '<b>0 to disable. Positive numbers are added to your Void Map zone. -1 for no cap.</b> This allows you to run new Void Maps obtained after your Void Map zone by adding this number to your Void Map zone. <br> <b>Example</b> Void map zone=187 and This setting=10. New Voids run until 197).<br>This means that any new void maps gained until Z197. CAUTION: May severely slow you down by trying to do too-high level void maps. Default 0 (OFF).', 'value', '0', null, 'Maps');
    ...
    createSetting('RMaxMapBonusAfterZone', '最大地图奖励起点', 'Always gets Max Map Bonus from this zone on. (inclusive and after).<br><b>NOTE:</b> Set -1 to disable entirely (default). Set 0 to use it always.', 'value', '-1', null, 'Maps');
    createSetting('RMaxMapBonuslimit', '最大地图奖励界限', 'Limit the amount of Map Bonuses you get. Default is 10. ', 'value', '10', null, 'Maps');
    createSetting('RMaxMapBonushealth', '最大地图奖励生命值', 'Limit the amount of map bonuses you get when AutoMaps requires more health. Default is 10. ', 'value', '10', null, 'Maps');
    createSetting('Rhitssurvived', '伤害承受次数', 'Set this value to tell the script how many enemy attacks you wish to survive for. The default is 10. The lower this is the less health the script will get. If you set this too high it will farm too much so please be careful. ', 'value', '10', null, 'Maps');
    createSetting('Rmapcuntoff', '地图界限', 'Decides when to get max map bonus. 1 is default. This means it will take 1 hit to kill an enemy. ', 'value', '1', null, 'Maps');
    createSetting('RDisableFarm', '刷资源命攻比', 'If H:D goes above this value, it will farm for Damage & Health. The lower this setting, the more it will want to farm. Default is <b>16</b>. <b>将数值设为-1则关闭刷资源模式！</b>', 'value', -1, null, 'Maps');

    //Line 2
    createSetting('RVoidMaps', '虚空地图', '<b>0 to disable</b> The zone at which you want all your void maps to be cleared inclusive of the zone you type. Runs them at Cell 70. Use odd zones on Lead.<br>', 'value', '0', null, "Maps");
    createSetting('Rvoidscell', '虚空格子', 'Run Voids at this Cell. -1 to run them at the default value, which is 70. ', 'value', '-1', null, 'Maps');
    createSetting('RRunNewVoidsUntilNew', '新虚空变化值', '<b>0 to disable. Positive numbers are added to your Void Map zone. -1 for no cap.</b> This allows you to run new Void Maps obtained after your Void Map zone by adding this number to your Void Map zone. <br> <b>Example</b> Void map zone=187 and This setting=10. New Voids run until 197).<br>This means that any new void maps gained until Z197. CAUTION: May severely slow you down by trying to do too-high level void maps. Default 0 (OFF).', 'value', '0', null, 'Maps');
    createSetting('Rprispalace', 'Prismatic Palace', 'Run Prismatic Palace when its unlocked. ', 'boolean', true, null, 'Maps');
    createSetting('Rmeltpoint', '熔点', '将数值设为-1则关闭该选项。设置运行熔点地图的时机。可以输入两个数值，格式类似50,91这样。第一个数字用于设置运行熔点地图的区域，第二个数字用于设置运行熔点地图的格子。前面的例子中，脚本将在区域50的格子91运行熔点地图。必须设置两个数值才能生效。用于熔化挑战和捕手道达人挑战。', 'multiValue', [-1], null, 'Maps');
    createSetting('Rfrozencastle', '非失温冻结城堡', '将数值设为-1则关闭该选项。设置运行冻结城堡地图的时机。可以输入两个数值，格式类似200,91这样。第一个数字用于设置运行冻结城堡地图的区域，第二个数字用于设置运行冻结城堡地图的格子。前面的例子中，脚本将在区域200的格子91运行冻结城堡地图。必须设置两个数值才能生效。用于失温挑战以外的情况，请小心设置。', 'multiValue', [-1], null, 'Maps');
    ...
    createSetting('Rtimefarmzone', '时刷：区域', 'Farms for specified minutes in TF: Time at zone according to this settings value. Can use 59,61,62. ', 'multiValue', [-1], null, 'Maps');
    createSetting('Rtimefarmcell', '时刷：格子', 'Time Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Maps');
    createSetting('Rtimefarmtime', '时刷：时间', 'How many minutes to farm at zone specified in TF. Can use 2,3,4. These values should match up to your TF zones. If using TF and TF: Times examples (59 and 2) it will farm at z59 for 2 minutes. ', 'multiValue', [-1], null, 'Maps');
    createSetting('Rtimemaplevel', '时刷：地图等级', 'What map level to use. Can use -1,1,2. -1 to use a level down from world (Map Reducer mastery gives loot equal to world one level down), 0 to use world, 1 etc to use +maps. Using 0 by itself will use global level for all maps. ', 'multiValue', [0], null, 'Maps');
    ...
    createSetting('Rtributefarmzone', '刷贡：区域', 'Farms for specified tributes in TrF: Tributes at zone according to this settings value. Can use 59,61,62. ', 'multiValue', [-1], null, 'Maps');
    createSetting('Rtributefarmcell', '刷贡：格子', 'Tribute Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Maps');
    createSetting('Rtributefarmamount', '刷贡：贡品数', 'How many tributes to farm at zone specified in TrF. Can use 2,3,4. These values should match up to your TrF zones. If using TrF and TrF: Tributes examples (59 and 1200) it will farm at z59 for 1200 tributes. ', 'multiValue', [-1], null, 'Maps');
    createSetting('Rtributemaplevel', '刷贡：地图等级', 'What map level to use. Can use -1,1,2. -1 to use a level down from world (Map Reducer mastery gives loot equal to world one level down), 0 to use world, 1 etc to use +maps. Using 0 by itself will use global level for all maps. ', 'multiValue', [0], null, 'Maps');
    ...
    createSetting('PreSpireNurseries', '进塔前托儿所', 'Set the maximum number of Nurseries to build for Spires. Overrides No Nurseries Until z and Max Nurseries so you can keep them seperate! Will build nurseries before z200 for Spire 1, but only on the zone of Spires 2+ to avoid unnecessary burning. Disable with -1.', 'value', -1, null, 'Spire');
    ...
    //Line 1
    createSetting('Praidingzone', '重铸劫掠区域', 'Raids Maps for prestiges at zone specified. Example: 495, will raid Maps at 501-505 sequentially. Once all gear is obtained from the maps, it will revert back to regular farming. Use P Raiding HD to determine how many extra maps you wish you raid. Extremely helpful for spire. Best used in poison zones. <b>You can use multiple values like this 495,506,525! </b>', 'multiValue', [-1], null, 'Raiding');
    createSetting('Praidingcell', '重铸劫掠格子', 'What Cell to start P Raiding at. Recommend below your BW Raiding cell if used together. -1 to Raid at cell 1. ', 'value', -1, null, 'Raiding');
    createSetting('PraidingHD', '重铸劫掠命攻比', 'Checks if you can raid the map. If your HD value (calculated using the maps you will raid) is below this value it will not buy the map and you will stop raiding. The higher this value the higher zones it will raid. Can raid up to +10 depending on the zone. -1 or 0 to remove this check.', 'value', -1, null, 'Raiding');
    createSetting('PraidingP', '重铸劫掠毒上限', 'Maximum level of map to P Raid at in Poison. If this value is 10 it will be able to go to +10 maps in Poison. You should use this instead of the HD function if you feel the calculations are off, but you can use both if needed. -1 or 0 to have no max. ', 'value', -1, null, 'Raiding');
    createSetting('PraidingI', '重铸劫掠冰上限', 'Maximum level of map to P Raid at in Ice. If this value is 10 it will be able to go to +10 maps in Ice. You should use this instead of the HD function if you feel the calculations are off, but you can use both if needed. -1 or 0 to have no max. ', 'value', -1, null, 'Raiding');
    ...
    createSetting('MaxPraidZone', '重铸劫掠最大区域', 'List of maximum zones to Praid corresponding to the list specified in Praiding zones.  e.g. if P raiding zones setting is 491,495 and this setting is 495,505, AT will P raid up to 495 from 491, and 505 from 495. Set to -1 to always buy highest available prestige map.  If no corrsponding value, or value is invalid, defaults to max available (up to +10)', 'multiValue', [-1], null, 'Raiding');
    createSetting('PraidFarmFragsZ', '刷碎片区域', 'P Raiding harder: List of zones where we should farm fragments until we can afford the highest or target prestige map for P raiding. Set to -1 to never farm fragments. ', 'multiValue', [-1], null, 'Raiding');
    createSetting('PraidBeforeFarmZ', '刷碎片前劫掠区域', 'P Raiding harder: List of zones where we should P Raid as far as we can afford before trying to farm fragments to Praid the highest or target prestige map.  Only occasionally useful, e.g. if it picks up a Speedexplorer or farming fragments is slow due to low damage. Set to -1 to never raid prestiges before farming fragents.', 'multiValue', [-1], null, 'Raiding');
    createSetting('BWraid', 'BW Raiding', 'Raids BW at zone specified in BW Raiding Z/max.', 'boolean', false, null, 'Raiding');
    createSetting('bwraidcell', '仿生劫掠格子', 'What Cell to start BW Raiding at. Recommend above your P Raiding cell if used together. -1 to Raid at cell 1. ', 'value', -1, null, 'Raiding');
    createSetting('BWraidingz', '仿生劫掠区域', 'Raids BWs at zone specified. Example: 495, will raid all BWs for all gear starting from 495. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Accepts comma separated lists, and raids up to the value in the corrsponding position in the Max BW to raid setting. So if this is set to 480,495 and Max BW to Raid is set to 500,515 AT will BW raid up to 500 from 480, and 515 from 495. Make sure these lists are the same length or BW raiding may fail.', 'multiValue', [-1], null, 'Raiding');
    createSetting('BWraidingmax', '仿生劫掠最大区域', 'Raids BWs until zone specified. Example: 515, will raid all BWs for all gear until 515. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Now accepts comma separated lists - see description of Z to BW raid setting for details.', 'multiValue', [-1], null, 'Raiding');

    
    //RRaiding

    //Line 1
    createSetting('RPraidingzone', '重铸劫掠区域', 'Raids Maps for prestiges at zone specified. Example: 495, will raid Maps at 501. Once all gear is obtained from the map, it will revert back to regular farming. Extremely helpful for spire. Best used in poison zones. <b>You can use multiple values like this 495,506,525! </b>', 'multiValue', [-1], null, 'Raiding');
    createSetting('RPraidingcell', '重铸劫掠格子', 'What Cell to start P Raiding at. Recommend below your BW Raiding cell if used together. -1 to Raid at cell 1. ', 'value', -1, null, 'Raiding');
    ...
    createSetting('RMaxPraidZone', '重铸劫掠最大区域', 'List of maximum zones to Praid corresponding to the list specified in Praiding zones.  e.g. if P raiding zones setting is 491,495 and this setting is 495,505, AT will P raid up to 495 from 491, and 505 from 495. Set to -1 to always buy highest available prestige map.  If no corrsponding value, or value is invalid, defaults to max available (up to +10)', 'multiValue', [-1], null, 'Raiding');
    createSetting('RPraidFarmFragsZ', '刷碎片区域', 'P Raiding harder: List of zones where we should farm fragments until we can afford the highest or target prestige map for P raiding. Set to -1 to never farm fragments. ', 'multiValue', [-1], null, 'Raiding');
    createSetting('RPraidBeforeFarmZ', '刷碎片前劫掠区域', 'P Raiding harder: List of zones where we should P Raid as far as we can afford before trying to farm fragments to Praid the highest or target prestige map.  Only occasionally useful, e.g. if it picks up a Speedexplorer or farming fragments is slow due to low damage. Set to -1 to never raid prestiges before farming fragents.', 'multiValue', [-1], null, 'Raiding');
    createSetting('RBWraid', 'BW Raiding', 'Raids BW at zone specified in BW Raiding Z/max.', 'boolean', false, null, 'Raiding');
    createSetting('RBWraidingz', '仿生劫掠区域', 'Raids BWs at zone specified. Example: 495, will raid all BWs for all gear starting from 495. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Accepts comma separated lists, and raids up to the value in the corrsponding position in the Max BW to raid setting. So if this is set to 480,495 and Max BW to Raid is set to 500,515 AT will BW raid up to 500 from 480, and 515 from 495. Make sure these lists are the same length or BW raiding may fail.', 'multiValue', [-1], null, 'Raiding');

    //Line 2
    createSetting('RBWraidingmax', '仿生劫掠最大区域', 'Raids BWs until zone specified. Example: 515, will raid all BWs for all gear until 515. Will skip lower BWs if you have enough damage. Once all gear is obtained, will return to regular farming. Now accepts comma separated lists - see description of Z to BW raid setting for details.', 'multiValue', [-1], null, 'Raiding');
    ...
    createSetting('RAMPraidzone', '二劫：区域', 'Zones to Prestige Raid. Can use 95,105,115! ', 'multiValue', [-1], null, 'Raiding');
    createSetting('RAMPraidraid', '二劫：地图', 'What Maps to Raid. Corrosponds to PR: Zone, so first value will corrospond to first value in PR: Zone. Can use 105,115,125! ', 'multiValue', [-1], null, 'Raiding');
    createSetting('RAMPraidcell', '二劫：格子', 'What Cell to start Prestige Raiding at. Recommend below your BW Raiding cell if used together. -1 to Raid at cell 1. ', 'value', -1, null, 'Raiding');
    ...
    createSetting('WindStackingMax', '风层数叠加层数', 'For use with Windstacking Stance. Amount of windstacks to obtain before switching to D stance. Default is 200, but I recommend anywhere between 175-190.  In Wind Enlightenment it will add 100 stacks to your total automatically. So if this setting is 200 It will assume you want 300 stacks instead during enlightenment. ', 'value', '200', null, 'Windstacking');
    createSetting('windcutoff', '风装备界限', 'Set this value to optimise your windstacking. Can work without AS3, but not recommended. AT normally uses 4 as its cutoff. I.e if the cutoff is above 4 it will buy max equipment. If you set this to 160, it will not get more damage till you are above x160. Essentially, the higher the value, the less damage AT wants to get, this will enable you to windstack to incredibly high amounts. -1 to disable/go back to default. Must set your windstacking min zone to use. ', 'value', '-1', null, 'Windstacking');
    createSetting('windcutoffmap', '风地图界限', 'Set this value to optimise your windstacking. Can work without AS3, but not recommended. AT normally uses 4 as its cutoff. I.e if the cutoff is above 4 it will get map bonus. If you set this to 160, it will not get more map bonus till you are above x160. Essentially, the higher the value, the less damage AT wants to get, this will enable you to windstack to incredibly high amounts. -1 to disable/go back to default. Must set your windstacking min zone to use. ', 'value', '-1', null, 'Windstacking');
    createSetting('wsmax', '风层数叠加上限', 'For maximising Windstacking an entire run. Withholds damage to try and get your max windstacks every wind zone. Not recommended for normal usage. Good for BPs. ', 'value', '-1', null, 'Windstacking');
    createSetting('wsmaxhd', '风层数叠加上限命攻比', 'Fiddle with this to maximise your WSM settings. Default is 0.00025. Same logic applies from the normal H:D setting. ', 'value', '-1', null, 'Windstacking');
    ...
    createSetting('ATGA2gen', '遗传：百分比', '<b>自动遗传学家：遗传学家百分比</b><br>ATGA will only hire geneticists if they cost less than this value. E.g if this setting is 1 it will only buy geneticists if they cost less than 1% of your food. Default is 1%. ', 'value', '1', null, 'ATGA');
    createSetting('ATGA2timer', '遗传：计时', '<b>ATGA Timer</b><br>This is the default time your ATGA will use. ', 'value', '-1', null, 'ATGA');

    //Zone Timers
    document.getElementById('ATGA2timer').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('zATGA2timer', '遗传：前区域', '<b>ATGA Timer: Before Z</b><br>ATGA will use the value you define in ATGA: T: BZT before the zone you have defined in this setting, overwriting your default timer. Useful for Liq or whatever. ', 'value', '-1', null, 'ATGA');
    createSetting('ztATGA2timer', '遗传：前区域计时', '<b>ATGA Timer: Before Z Timer</b><br>ATGA will use this value before the zone you have defined in ATGA: T: Before Z, overwriting your default timer. Useful for Liq or whatever. Does not work on challenges. ', 'value', '-1', null, 'ATGA');
    createSetting('ATGA2timerz', '遗传：后区域', '<b>ATGA Timer: After Z</b><br>ATGA will use the value you define in ATGA: T: AZT after the zone you have defined in this setting, overwriting your default timer. Useful for super push runs or whatever. Does not work on challenges. ', 'value', '-1', null, 'ATGA');
    createSetting('ATGA2timerzt', '遗传：后区域计时', '<b>ATGA Timer: After Z Timer</b><br>ATGA will use this value after the zone that has been defined in ATGA: T: After Z, overwriting your default timer. Useful for super push runs or whatever. ', 'value', '-1', null, 'ATGA');

    //Spire Timers
    document.getElementById('ATGA2timerzt').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('sATGA2timer', '遗传：尖塔', '<b>ATGA Timer: Spire</b><br>ATGA will use this value in Spires. Respects your ignore Spires setting. Do not use this if you use the setting in the Spire tab! (As that uses vanilla GA) Nothing overwrites this except Daily Spire. ', 'value', '-1', null, 'ATGA');
    createSetting('dsATGA2timer', '遗传：日常尖塔', '<b>ATGA Timer: Daily Spire</b><br>ATGA will use this value in Daily Spires. Respects your ignore Spires setting. Do not use this if you use the setting in the Spire tab! (As that uses vanilla GA) Nothing overwrites this. ', 'value', '-1', null, 'ATGA');
    ...
    createSetting('dATGA2timer', '遗传：日常', '<b>ATGA Timer: Normal Dailies</b><br>ATGA will use this value for normal Dailies such as ones without plague etc, overwriting your default timer. Useful for pushing your dailies that extra bit at the end. Overwrites Default, Before Z and After Z. ', 'value', '-1', null, 'ATGA');
    createSetting('dhATGA2timer', '遗传：困难日常', '<b>ATGA Timer: Hard Dailies</b><br>ATGA will use this value in Dailies that are considered Hard. Such Dailies include plaged, bloodthirst and Dailies with a lot of negative mods. Overwrites Default, Before Z and After Z and normal Daily ATGA Timer. ', 'value', '-1', null, 'ATGA');

    //C2 Timers
    document.getElementById('dhATGA2timer').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('cATGA2timer', '遗传：挑战²', '<b>自动遗传学家计时：普通挑战²</b><br>ATGA will use this value in C2s. Overwrites Default, Before Z and After Z. ', 'value', '-1', null, 'ATGA');
    createSetting('chATGA2timer', '遗传：困难挑战²', '<b>自动遗传学家计时：困难挑战²</b><br>ATGA will use this value in C2s that are considered Hard. Electricity, Nom, Toxicity. Overwrites Default, Before Z and After Z and C2 ATGA', 'value', '-1', null, 'ATGA');
    ...
    createSetting('Rblackbogzone', '区域', 'What zones to run Black Bogs at. Can use 40,50,60. ', 'multiValue', [-1], null, 'Challenges');
    createSetting('Rblackbogamount', '数量', 'How many Black Bogs to at specified zones. Can use 8,9,10. I.e if BB: Zone had 40,50 and this setting had 8,10, It would run 8 Black Bogs at z40 and 10 Black Bogs at z50. ', 'multiValue', [-1], null, 'Challenges');
    ...
    createSetting('Rarchstring1', '第一字符串', 'First string to use in Archaeology. Put the zone you want to stop using this string and start using the second string (Make sure the second string has a value) at the start. I.e: 70,10a,10e ', 'textValue', 'undefined', null, 'Challenges');
    createSetting('Rarchstring2', '第二字符串', 'Second string to use in Archaeology. Put the zone you want to stop using this string and start using the third string (Make sure the third string has a value) at the start. I.e: 94,10a,10e ', 'textValue', 'undefined', null, 'Challenges');
    createSetting('Rarchstring3', '第三字符串', 'Third string to use in Archaeology. Make sure this is just your Archaeology string and nothing else. I.e: 10a,10e ', 'textValue', 'undefined', null, 'Challenges');
    ...
    createSetting('Rmayhemabcut', '暴乱：攻击首领界限', 'What cut-off to use when farming for the boss using M: Attack. If this setting is 100, the script will farm till you can kill the boss in 100 average hits. ', 'value', '-1', null, 'Challenges');
    createSetting('Rmayhemamcut', '暴乱：地图界限', 'What cut-off to use when farming maps using M: Attack and M: Smart Map. If this setting is 10, the script will do maps you can kill cells in 10 average hits. ', 'value', '-1', null, 'Challenges');
    createSetting('Rmayhemhcut', '暴乱：生命值界限', 'What cut-off to use when using M: Health. ', 'value', '-1', null, 'Challenges');
    ...
    createSetting('Rstormzone', '风暴：区域', 'What zone to start S: H:D and S: Multiplier. ', 'value', '-1', null, 'Challenges');
    createSetting('RstormHD', '风暴：命攻比', 'What H:D to use inside Storm. ', 'value', '-1', null, 'Challenges');
    createSetting('Rstormmult', '风暴：倍率', 'Starting from the zone above S: Zone, this setting will multiply the H:D you have set in S: H:D. So if S: Zone was 100, S: H:D was 10, S: Multiplier was 1.2, at z101 your H:D target will be 12, then at z102 it will be 14.4 and so on. This way you can account for the zones getting stronger and you will not waste time farming for a really low H:D. ', 'value', '-1', null, 'Challenges');
    ...
    createSetting('Rinsanityfarmzone', '刷失智层数', 'Farms for specified stacks in IF: Stacks at zone according to this settings value. Can use 108,109,110. ', 'multiValue', [-1], null, 'Challenges');
    createSetting('Rinsanityfarmcell', '刷失智层数：格子', 'Insanity Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Challenges');
    createSetting('Rinsanityfarmstack', '刷失智层数：层数', 'How many stacks to farm at zone specified in IF. Can use 300,400,500. These values should match up to your IF zones. If using IF and IF: Stacks examples (110 and 500) it will farm at z110 for 500 stacks of insanity. If stacks specified are above maximum Insanity Stacks, it will farm for maximum. ', 'multiValue', [-1], null, 'Challenges');
    createSetting('Rinsanityfarmlevel', '刷失智层数：地图等级', 'What map level to use. Can use -1,1,2. -1 to use a level down from world (This is how to remove insanity stacks), 0 to use world, 1 etc to use +maps. Using 0 by itself will use global level for all maps. ', 'multiValue', [0], null, 'Challenges');
    ...
    createSetting('Rpandazone', '群魔乱舞：区域', 'What zone to start Pandemonium mapping at. Will ignore Pandemonium stacks below this zone. ', 'value', '-1', null, 'Challenges');
    createSetting('Rpandahits', '群魔乱舞：攻击次数', 'How many hits an enemy in a plus map should take to kill. Will select up to +6 levels. If you cannot kill an enemy in the maximum number of hits in any plus map, will try to run a +1 map anyway. ', 'value', '-1', null, 'Challenges');
    ...
    createSetting('Ralchfarmzone', '刷炼金术', 'Farms for specified potion levels in AF: Potions at zone according to this settings value. Can use 138,139,140. ', 'multiValue', [-1], null, 'Challenges');
    createSetting('Ralchfarmcell', '刷炼金术：格子', 'Alchemy Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Challenges');
    createSetting('Ralchfarmstack', '刷炼金术：药剂', 'How many levels of a potion to farm at zone specified in AF. You must pair a potion with a level here. Example: h15\,g20\,s15. This will farm Herby potion up to level 15 on the first AF zone\, Gaseous potion to level 20 and so on. It will also buy that potion so don\'t bother using the vanilla autobrew. Think of it like an Arch string\, you remember that challenge right?', 'textValue', 'undefined', null, 'Challenges');
    createSetting('Ralchfarmlevel', '刷炼金术：地图等级', 'What map level to use. Can use -1,1,2. -1 to use a level down from world, 0 to use world, 1 etc to use +maps. Using 0 by itself will use global level for all maps. ', 'multiValue', [0], null, 'Challenges');
    createSetting('Ralchfarmselection', '刷炼金术：选择地图', 'Select which map you prefer to use for AF. Use m = Mountains\, f = Forest\, s = Sea\, d = Depths\, g = Gardens\, l = farmLands. ', 'textValue', 'l', null, 'Challenges');
    ...
    createSetting('Rhypofarmzone', '刷失温', 'Farms for specified total bonfires in HF: Bonfire at zone according to this settings value. Can use 138,139,140. ', 'multiValue', [-1], null, 'Challenges');
    createSetting('Rhypofarmcell', '刷失温：格子', 'Hypothermia Farm at this Cell. -1 to run them at the default value, which is 1. ', 'value', '-1', null, 'Challenges');
    createSetting('Rhypofarmstack', '刷失温：篝火', 'How many Bonfires total you have earned throughout the run you wish to have at the corrosponding level in Hypothermia Farming. You may have to keep on eye on bonfires at start of run to get this number. With packrat you normally get 5 total before storage is too small. So just start with 6\,7\,8\,9\,10... and so on then just add numbers all the way up to something like 30\, this would cover all scenarios anyway. ', 'multiValue', 'undefined', null, 'Challenges');
    ...
    createSetting('AutoRoboTrimp', '自动脆皮机器人', 'Use RoboTrimps ability starting at this level, and every 5 levels thereafter. (set to 0 to disable. default 60.) 60 is a good choice for mostly everybody.', 'value', '60', null, 'Combat');

    //Line 2
    createSetting('fightforever', '常时战斗', 'U1: -1 to disable. Sends trimps to fight if they\'re not fighting, regardless of BAF. Has 2 uses. Set to 0 to always send out trimps. Or set a number higher than 0 to enable the H:D function. If the H:D ratio is below this number it will send them out. I.e, this is set to 1, it will send out trimps regardless with the H:D ratio is below 1. ', 'value', '-1', null, 'Combat');
    ...
    //RCombat
    createSetting('Rfightforever', '常时战斗', 'U2: -1 to disable. Sends trimps to fight if they\'re not fighting, regardless of BAF. Has 2 uses. Set to 0 to always send out trimps. Or set a number higher than 0 to enable the H:D function. If the H:D ratio is below this number it will send them out. I.e, this is set to 1, it will send out trimps regardless with the H:D ratio is below 1. ', 'value', '-1', null, 'Combat');
    ...
    createSetting('ScryerMinZone', '最低区域', 'Minimum zone to start using scryer in.(inclusive) Recommend:(60 or 181). Overkill ignores this. This needs to be On & Valid for the <i>MAYBE</i> option on all other Scryer settings to do anything if Overkill is off. Tip: Use 9999 to disable all Non-Overkill, Non-Force, scryer usage.', 'value', '181', null, 'Scryer');
    createSetting('ScryerMaxZone', '最高区域', '<b>0 or -1 to disable (Recommended)</b><br>Overkill ignores this. Zone to STOP using scryer at (not inclusive). Turning this ON with a positive number stops <i>MAYBE</i> use of all other Scryer settings.', 'value', '230', null, 'Scryer');
    ...
    createSetting('ScryUseinPoison', '毒区域占卜者阵型', 'Decides what you do in Poison. <br><b>-1</b> = Maybe <br><b>0</b> = Never <br><b>超过0</b> = Max Zone you want it scrying', 'value', -1, null, 'Scryer');

    //Line 3
    createSetting('ScryUseinWind', '风区域占卜者阵型', 'Decides what you do in Wind. <br><b>-1</b> = Maybe <br><b>0</b> = Never <br><b>超过0</b> = Max Zone you want it scrying', 'value', -1, null, 'Scryer');
    createSetting('ScryUseinIce', '冰区域占卜者阵型', 'Decides what you do in Ice. <br><b>-1</b> = Maybe <br><b>0</b> = Never <br><b>超过0</b> = Max Zone you want it scrying', 'value', -1, null, 'Scryer');
    createSetting('ScryerDieZ', '放弃士兵使用占卜者阵型', '<b>设为-1则关闭该选项。</b><br>Turning this on will switch you back to S even when doing so would kill you. Happens in scenarios where you used Skip Corrupteds that took you into regular Autostance X/H stance, killed the corrupted and reached a non-corrupted enemy that you wish to use S on, but you havent bred yet and you are too low on health to just switch back to S. So you\'d rather die, wait to breed, then use S for the full non-corrupted enemy, to maximize DE. NOTE: Use at your own risk.<br>Use this input to set the minimum zone that scryer activates in (You can use decimal values to specify what cell this setting starts from)', 'value', 230.60, null, 'Scryer');
    ...
    createSetting('fuellater', '持续得燃料起始区域', 'Start fueling at this zone instead of 230. I would suggest you have a value lower than your max, for obvious reasons. Recommend starting at a value close-ish to your max supply. Use 230 to use your <b>BEFORE FUEL</b> setting. ', 'value', -1, null, 'Magma');
    createSetting('fuelend', '持续得燃料结束区域', 'End fueling at this zone. After this zone is reached, will follow your preference. -1 to fuel infinitely. ', 'value', -1, null, 'Magma');
    ...
    //Heirloom
    createSetting('highdmg', '风层数叠加：高伤', '<b>HIGH DAMAGE HEIRLOOM</b><br><br>Enter the name of your high damage heirloom. This is your heirloom that you will use normally. ', 'textValue', 'undefined', null, 'Heirlooms');
    createSetting('lowdmg', '风层数叠加：低伤', '<b>LOW DAMAGE HEIRLOOM</b><br><br>Enter the name of your low damage heirloom. This is the heirloom that you will use for windstacking. ', 'textValue', 'undefined', null, 'Heirlooms');
	...
	createSetting('Rhsz', '传换：区域', '切换盾牌的区域。例如：将该选项设为75，则在<b>区域75</b>之前使用“传换：切换前”名字的盾牌，之后使用“传换：切换后”名字的盾牌。', 'value', '-1', null, 'Heirlooms');
	createSetting('Rhs1', '传换：切换前', '<b>First Heirloom to use</b><br><br>Enter the name of your first heirloom. This is the heirloom that you will use before swapping to the second heirloom at the zone you have defined in the HS: Zone. ', 'textValue', 'undefined', null, 'Heirlooms');
	createSetting('Rhs2', '传换：切换后', '<b>Second Heirloom to use</b><br><br>Enter the name of your second heirloom. This is the heirloom that you will use after swapping from the first heirloom at the zone you have defined in the HS: Zone. ', 'textValue', 'undefined', null, 'Heirlooms');
	...
	createSetting('Rhsworldstaff', '传换：世界时', '<b>World Staff</b><br><br>Enter the name of your world staff.', 'textValue', 'undefined', null, 'Heirlooms');
	createSetting('Rhsmapstaff', '传换：地图时', '<b>Mapping staff</b><br><br>Enter the name of your mapping staff.', 'textValue', 'undefined', null, 'Heirlooms');
	createSetting('Rhstributestaff', '传换：刷贡品', '<b>Tribute farming staff</b><br><br>Enter the name of the staff you would like to equip during tribute farming.', 'textValue', 'undefined', null, 'Heirlooms');
    ...
    createSetting('slot1modst', '权杖：词缀1', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 1 is worth 7 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot2modst', '权杖：词缀2', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 2 is worth 6 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot3modst', '权杖：词缀3', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 3 is worth 5 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot4modst', '权杖：词缀4', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 4 is worth 4 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot5modst', '权杖：词缀5', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 5 is worth 3 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot6modst', '权杖：词缀6', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 6 is worth 2 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    createSetting('slot7modst', '权杖：词缀7', 'Auto Heirlooms. Keeps Staffs with selected Mod. Modifier 7 is worth 1 points. ', 'dropdown', 'empty', ["empty", "metalDrop", "foodDrop", "woodDrop", "gemsDrop", "fragmentsDrop", "minerSpeed", "FarmerSpeed", "LumberjackSpeed", "DragimpSpeed", "ExplorerSpeed", "ScientistSpeed", "FluffyExp", "ParityPower"], 'Heirlooms');
    ...
    createSetting('slot1modcr', '核心：词缀1', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 1 is worth 5 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms');
    createSetting('slot2modcr', '核心：词缀2', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 2 is worth 4 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms');
    createSetting('slot3modcr', '核心：词缀3', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 3 is worth 3 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms');
    createSetting('slot4modcr', '核心：词缀4', 'Auto Heirlooms. Keeps Cores with selected Mod. Modifier 4 is worth 2 points. ', 'dropdown', 'empty', ["empty", "fireTrap", "poisonTrap", "lightningTrap", "runestones", "strengthEffect", "condenserEffect"], 'Heirlooms');
    ...
    createSetting('voidheliumbattle', '虚空战斗切换区域', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Helium at this zone and onwards. This option only appears when selecting void. ', 'value', -1, null, 'Golden');
    createSetting('dvoidheliumbattle', '日常虚空战斗切换区域', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Helium at this zone and onwards in Dailies. This option only appears when selecting void. ', 'value', -1, null, 'Golden');
    createSetting('radonbattle', '氦战斗切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of helium after this many helium goldens have been purchased and onwards. This option only appears when selecting helium. ', 'value', -1, null, 'Golden');
    createSetting('dradonbattle', '日常氦战斗切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of helium after this many helium goldens have been purchased and onwards in Dailies. This option only appears when selecting helium. ', 'value', -1, null, 'Golden');
    createSetting('battleradon', '战斗氦切换次数', '<b>设为-1则关闭该选项。</b><br> Buys helium goldens instead of Battle after this many Battle goldens have been purchased and onwards. This option only appears when selecting battle. ', 'value', -1, null, 'Golden');
    createSetting('dbattleradon', '日常战斗氦切换次数', '<b>设为-1则关闭该选项。</b><br> Buys helium goldens instead of Battle after this many battle goldens have been purchased and onwards in Dailies. This option only appears when selecting battle. ', 'value', -1, null, 'Golden');
    ...
    createSetting('Rvoidheliumbattle', '虚空战斗切换区域', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Radon at this zone and onwards. This option only appears when selecting void. ', 'value', -1, null, 'Golden');
    createSetting('Rdvoidheliumbattle', '日常虚空战斗切换区域', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Radon at this zone and onwards in Dailies. This option only appears when selecting void. ', 'value', -1, null, 'Golden');
    createSetting('Rradonbattle', '氡战斗切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Radon after this many Radon goldens have been purchased and onwards. This option only appears when selecting radon. ', 'value', -1, null, 'Golden');
    createSetting('Rdradonbattle', '日常氡战斗切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Battle goldens instead of Radon after this many Radon goldens have been purchased and onwards in Dailies. This option only appears when selecting radon. ', 'value', -1, null, 'Golden');
    createSetting('Rbattleradon', '战斗氡切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Radon goldens instead of Battle after this many Battle goldens have been purchased and onwards. This option only appears when selecting battle. ', 'value', -1, null, 'Golden');
    createSetting('Rdbattleradon', '日常战斗氡切换次数', '<b>设为-1则关闭该选项。</b><br> Buys Radon goldens instead of Battle after this many battle goldens have been purchased and onwards in Dailies. This option only appears when selecting battle. ', 'value', -1, null, 'Golden');
    ...
    createSetting('RABfarmstring', '尖塔突击字符串', 'This is your best farming string. Feel free to share it with other AT users. If you do use a shared string I advise you to change the second value (the dust part) to 0 so it calcs the actual dust you get instead of the shared strings. ', 'textValue', '-1', null, "SA");
    ...
    createSetting('tokenthresh', '符记阈值', 'If Tokens would go below this value it will not convert tokens. ', 'value', -1, null, 'Nature');
    createSetting('AutoPoison', '毒', 'Spend/convert Poison tokens', 'dropdown', 'Off', ['Off', 'Empowerment', 'Transfer', 'Convert to Wind', 'Convert to Ice', 'Convert to Both'], 'Nature');
    ...
    createSetting('pfillerenlightthresh', '毒之启迪阈值', 'Activate Poison Enlight when Enlight cost is below this Thresh in Fillers. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('wfillerenlightthresh', '风之启迪阈值', 'Activate Wind Enlight when Enlight cost is below this Thresh in Fillers. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('ifillerenlightthresh', '冰之启迪阈值', 'Activate Ice Enlight when Enlight cost is below this Thresh in Fillers. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    document.getElementById('ifillerenlightthresh').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('pdailyenlightthresh', '日常毒之启迪阈值', 'Activate Poison Enlight when Enlight cost is below this Thresh in Dailies. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('wdailyenlightthresh', '日常风之启迪阈值', 'Activate Wind Enlight when Enlight cost is below this Thresh in Dailies. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('idailyenlightthresh', '日常冰之启迪阈值', 'Activate Ice Enlight when Enlight cost is below this Thresh in Dailies. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    document.getElementById('idailyenlightthresh').parentNode.insertAdjacentHTML('afterend', '<br>');
    createSetting('pc2enlightthresh', '挑战²毒之启迪阈值', 'Activate Poison Enlight when Enlight cost is below this Thresh in C2s. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('wc2enlightthresh', '挑战²风之启迪阈值', 'Activate Wind Enlight when Enlight cost is below this Thresh in C2s. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    createSetting('ic2enlightthresh', '挑战²冰之启迪阈值', 'Activate Ice Enlight when Enlight cost is below this Thresh in C2s. Consumes Tokens. -1 to disable. ', 'value', -1, null, 'Nature');
    ...
    createSetting('Export60', '区域60以下的脚本设置', 'Gives you an AT settings String that you can use to import. Use if you are less than z60. May not work for you perfectly, should really only be used as an example!', 'infoclick', 'Export60', null, 'Import Export');
...
function autoSetValue(id,negative, multi) {
    ...
    if (numBox.value) {
        ...
        if (multi) {
            ...
            if (!isNumberBad(num) || (num == -1 || num < 0 && negative))
               num = parseNum(num);
            else
               return;
        }
            ...
            if (elem != null) {
                if (item.type == 'multitoggle'){
                    ...
                    if (Array.isArray(item.value) && item.value.length == 1 && item.value[0] == -1){
                        ...
                        if (elem.dataset["oritext"] != item.name + '：' + item.value.toString().split(",")[0] + '+'){
                            elem.dataset["oritext"] = item.name + '：' + item.value.toString().split(",")[0] + '+';
                            elem.textContent = item.name + '：' + item.value.toString().split(",")[0] + '+';
                        }
                    }
                }
